  ...
  describe("TotallyNotMutable autofreeze:" + autoFreeze, () => {
    describe("mergeIn", () => {
      ...
      it("nested object", () => {
        ...
        const mergedInValue = tnm.apply(init);
        expect(mergedInValue === init).toEqual(true);
        const lastValue = tnm.mutate((val) => (val.obj1.obj2.obj3.foo = false));
        expect(mergedInValue === lastValue).toEqual(false);
        expect(mergedInValue.obj2 === lastValue.obj2).toEqual(true);
      });
