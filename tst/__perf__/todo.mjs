...
/**
 * These tests are strictly to test the speed of the modifications, not the time it takes to read the data or create the initial state
 */
...
measure("immutableJS", () => {
  immutableJsBaseState.withMutations((state) => {
    for (let i = 0; i < MAX * MODIFY_FACTOR; i++) {
 ...
 * IMMER
 *
 * Note: Immer's produce always has to read the data in first, so Immer's tests will have the measure INSIDE the produce draft handler
 */
//IMMER
...
produce(
  baseState,
  (draft) => {
    measure("immer - without autofreeze - with patch listener", () => {
      for (let i = 0; i < MAX * MODIFY_FACTOR; i++) {
        draft[i].done = true;
      }
    });
...
setAutoFreeze(true);
produce(baseState, (draft) => {
  measure("immer - with autofreeze - with patch listener", () => {
    for (let i = 0; i < MAX * MODIFY_FACTOR; i++) {
      draft[i].done = true;
    }
  });
...
measure(
  "TotallyNotMutable w/autofreeze",
